...
//import com.examples.empapp.service.EmployeeServiceColImpl;
import com.examples.empapp.service.EmployeeServiceHibernet;
import com.examples.empapp.service.EmployeeServiceHibernet_Jpa;
...
public class EmployeeManagementApp {
	...
	private static EmployeeDao emphiber;
	private static EmployeeDao emphiberjpa;
...
//		empDao = new EmployeeDaoImpl();
	//	emphiber =new  EmployeeServiceHibernet();
		
		emphiberjpa = new  EmployeeServiceHibernet_Jpa();
		...
		while (true) {
			...
			try {
				...
				case 2:
					...
					empId = in.nextInt();
					Employee emp = null;
				//	emp = empDao.viewEmp(empId); //Calling ViewEmp Method of DAO Class
					emp=emphiber.viewEmp(empId);// calling view method of employeeservicehiber class
					printHeader();
					printDetail(emp);
					break;
				case 3:
					...
					empForUpdate=emphiber.viewEmp(empId);
					captureEmpDetail(empForUpdate);
				//	empDao.update(empForUpdate);
					emphiber.update(empForUpdate);
...
//					
	...
	private static void printStatistics() {
		...
		//		+ empService.getEmployeeCountAgeGreaterThan(30));
				+ emphiber.getEmployeeCountAgeGreaterThan(30));
		System.out.println("List employee IDs older than thirty years: "
				//+ empService.getEmployeeIdsAgeGreaterThan(30));
				+ emphiber.getEmployeeCountAgeGreaterThan(30));
		System.out.println("Employee count by Department: " 
				//+ empService.getEmployeeCountByDepartment());
				+ emphiber.getEmployeeCountByDepartment());
		System.out.println("Employee count by Department ordered: " 
				//+ empService.getEmployeeCountByDepartmentOdered());
				+ emphiber.getEmployeeCountByDepartmentOdered());
	}
	...
	private static void addEmployee() throws NumberFormatException {
		...
		//emphiber.insert(employee);
		emphiberjpa.insert(employee);
	}
